...
namespace Bifrost.Configuration
{
    ...
    public static partial class ConfigurationExtensions
    {
        ...
        {
            eventsConfiguration.EventStoreType = typeof(FileEventStore);
            eventsConfiguration.EventSubscriptionsType = typeof(FileEventSubscriptions);
            eventsConfiguration.UncommittedEventStreamCoordinatorType = typeof(UncommittedEventStreamCoordinator);

            if (!Path.IsPathRooted(path))
                path = Path.Combine(Directory.GetCurrentDirectory(), path);

            var configuration = new FileEventStoreConfiguration
            {
                Path = path
            };
            Configure.Instance.Container.Bind<FileEventStoreConfiguration>(configuration);
