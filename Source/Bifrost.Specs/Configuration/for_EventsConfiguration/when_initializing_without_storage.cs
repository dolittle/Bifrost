...
namespace Bifrost.Specs.Configuration.for_EventsConfiguration
{
    ...
    public class when_initializing_without_storage : given.an_events_configuration_and_container_object
    {
        ...
        It should_not_set_up_storage_for_events = () => container_mock.Verify(c => c.Bind(typeof(IEntityContext<EventHolder>), Moq.It.IsAny<Type>()), Times.Never());
        It should_not_set_up_storage_for_event_subscriptions = () => container_mock.Verify(c => c.Bind(typeof(IEntityContext<EventSubscriptionHolder>), Moq.It.IsAny<Type>()), Times.Never());
