...
namespace Bifrost.Specs.Domain.for_AggregatedRootRepository
{
    ...
    public class when_getting_a_stateless_aggregated_root_that_has_events : given.a_repository_for_a_stateless_aggregated_root
    {
        ...
        Establish context = () =>
                                {
                                    ...
                                    event_store_mock.Setup(e => e.GetLastCommittedVersion(Moq.It.IsAny<EventSource>(),aggregated_root_id)).Returns(version_of_last_event);
                                };
        ...
        It should_not_get_events_for_the_aggregated_root = () => event_store_mock.Verify(e => e.GetForEventSource(Moq.It.IsAny<EventSource>(), aggregated_root_id), Moq.Times.Never());
        ...
        It should_ensure_the_event_source_has_the_correct_version = () =>
                                                                        {
                                                                            event_store_mock.Verify(e => e.GetLastCommittedVersion(Moq.It.IsAny<EventSource>(), aggregated_root_id), Moq.Times.Once());
