...
namespace Bifrost.Specs.Strings.for_StringFormatBuilder
{
    public class when_building_recursed_variable_string : given.an_empty_string_format_builder
    {
        ...
        Establish context = () => resulting_builder = builder.VariableString(variable_string, f => f.Recurse());

        Because of = () => string_format = resulting_builder.Build();

        It should_hold_a_recursed_segment = () => ((VariableStringSegment) string_format.Segments.First()).Occurrences.ShouldEqual(SegmentOccurence.Recurring);
    }
}
